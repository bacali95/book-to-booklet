...
export function BookViewTab({ pageCount, direction, sourcePages }: Props) {
  ...
  const next    = useCallback(() => setIdx(i => Math.min(spreads.length - 1, i + 1)), [spreads.length])
  const first   = useCallback(() => setIdx(0), [])
  const last    = useCallback(() => setIdx(spreads.length - 1), [spreads.length])

  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft')  { e.preventDefault(); direction === 'rtl' ? next() : prev() }
      if (e.key === 'ArrowRight') { e.preventDefault(); direction === 'rtl' ? prev() : next() }
  ...
  const dirLabel = direction === 'rtl'
    ? '→ Right-to-Left (RTL) · spine on right'
    : '← Left-to-Right (LTR) · spine on left'

  const spreadLabel = spread.isCover ? 'Front Cover'
    : spread.isBack ? 'Back Cover'
    : direction === 'rtl'
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-2">
      ...
      <div
        ...
        style={{
          boxShadow: '0 2px 4px rgba(0,0,0,.12), 0 6px 18px rgba(0,0,0,.22), 0 20px 50px rgba(0,0,0,.18)',
      ...
      >
        ...
        >
          <BookHalf pageIdx={spread.left} sourcePages={sourcePages} width={W} height={H} isDark={isDark} />
          {spread.left >= 0 && (
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(to right, #a0a0a0 0%, #d8d8d8 35%, #e8e8e8 50%, #c8c8c8 70%, #909090 100%)',
            height: H,
          }}
        />
        ...
        >
          <BookHalf pageIdx={spread.right} sourcePages={sourcePages} width={W} height={H} isDark={isDark} />
          {spread.right >= 0 && (
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 text-[10px] text-muted-foreground">
              p.{spread.right + 1}
            </span>
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" variant="outline" onClick={first} disabled={idx === 0} title="First spread">⏮</Button>
        <Button variant="outline" onClick={direction === 'rtl' ? next : prev} disabled={idx === 0}>← Prev</Button>
        <span className="text-sm text-muted-foreground min-w-[160px] text-center">
          {idx + 1} / {spreads.length} — {spreadLabel}
        </span>
        <Button variant="outline" onClick={direction === 'rtl' ? prev : next} disabled={idx === spreads.length - 1}>Next →</Button>
        <Button size="sm" variant="outline" onClick={last} disabled={idx === spreads.length - 1} title="Last spread">⏭</Button>
      </div>

      <p className="text-xs text-muted-foreground">Use ← → arrow keys or click to navigate</p>
    </div>
  )
}
