...
function buildInnerSheets(origIndices: number[], S: number, direction: Direction): {
  sheets: Sheet[]; padded: number; numSig: number; shPerSig: number
} {
...
export function computeLayout(pageCount: number, opts: LayoutOptions = {}): BookletLayout {
  ...
  if (separateCover && pageCount >= 2) {
    const coverFront: SheetSide = direction === 'rtl'
    ...
    return toResult(pageCount, padded + 2, numSig, shPerSig, direction, [coverSheet, ...inner])
  }

  const { sheets, padded, numSig, shPerSig } =
    buildInnerSheets(Array.from({ length: pageCount }, (_, i) => i), S, direction)
  return toResult(pageCount, padded, numSig, shPerSig, direction, sheets)
}

export function creepOffset(sheetDepth: number, sheetsPerSig: number, creepPerSheetMm: number): number {
